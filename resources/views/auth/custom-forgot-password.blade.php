...
<head>
  ...
  <title>{{ __('civinsis.auth.forgot_titulo') }} – CIVINSIS</title>
  ...
  <main class="auth-form-side">
    ...
    <div class="auth-form-wrap">
      ...
      <div class="form-panel is-active">
        <div class="form-head">
          <h1><i class="fas fa-key" style="color:var(--c-teal);margin-right:.4rem"></i>{{ __('civinsis.auth.forgot_titulo') }}</h1>
          <p>{{ __('civinsis.auth.forgot_subtitulo') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('custom.password.email') }}">
          ...
          <div class="form-group">
            ...
            <div class="input-wrap">
              ...
              <input class="field" type="email" id="email" name="email" value="{{ old('email') }}" placeholder="tu@correo.com" autocomplete="email" autofocus required>
              <div class="focus-line"></div>
            </div>
          </div>

          <button type="submit" class="submit-btn">
            <span>{{ __('civinsis.auth.forgot_boton') }}</span> <i class="fas fa-arrow-right arrow"></i>
          </button>
        </form>

        <p class="form-switch">
          <a href="{{ url('/auth.php') }}" class="switch-btn" style="text-decoration:none">{{ __('civinsis.auth.forgot_volver') }}</a>
        </p>
      </div>
