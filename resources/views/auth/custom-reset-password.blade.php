...
<head>
  ...
  <title>{{ __('civinsis.auth.reset_titulo') }} – CIVINSIS</title>
  ...
  <main class="auth-form-side">
    ...
    <div class="auth-form-wrap">
      ...
      <div class="form-panel is-active">
        <div class="form-head">
          <h1><i class="fas fa-lock-open" style="color:var(--c-teal);margin-right:.4rem"></i>{{ __('civinsis.auth.reset_titulo') }}</h1>
          <p>{{ __('civinsis.auth.reset_subtitulo') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('custom.password.update') }}">
          ...
          <div class="form-group">
            <label class="form-label" for="email">{{ __('civinsis.auth.correo_electronico') }}</label>
            <div class="input-wrap">
              <i class="ico fas fa-envelope"></i>
              <input class="field" type="email" id="email" name="email" value="{{ $email ?? old('email') }}" readonly>
              <div class="focus-line"></div>
            </div>
          </div>

          <div class="form-group">
            <label class="form-label" for="password">{{ __('civinsis.auth.reset_campo_nueva') }}</label>
            <div class="input-wrap">
              <i class="ico fas fa-lock"></i>
              <input class="field" type="password" id="password" name="password" placeholder="{{ __('civinsis.auth.reset_campo_nueva_placeholder') }}" minlength="8" autocomplete="new-password" autofocus required>
              <button type="button" class="eye-btn" data-for="password" aria-label="{{ __('civinsis.auth.mostrar_contrasena') }}"><i class="fas fa-eye"></i></button>
              <div class="focus-line"></div>
            </div>
          </div>

          <div class="form-group">
            <label class="form-label" for="password_confirmation">{{ __('civinsis.auth.reset_campo_confirmar') }}</label>
            <div class="input-wrap">
              <i class="ico fas fa-lock"></i>
              <input class="field" type="password" id="password_confirmation" name="password_confirmation" placeholder="{{ __('civinsis.auth.reset_campo_confirmar_placeholder') }}" minlength="8" autocomplete="new-password" required>
              <button type="button" class="eye-btn" data-for="password_confirmation" aria-label="{{ __('civinsis.auth.mostrar_contrasena') }}"><i class="fas fa-eye"></i></button>
              <div class="focus-line"></div>
            </div>
          </div>

          <button type="submit" class="submit-btn">
            <span>{{ __('civinsis.auth.reset_boton') }}</span> <i class="fas fa-arrow-right arrow"></i>
          </button>
        </form>

        <p class="form-switch">
          <a href="{{ url('/auth.php') }}" class="switch-btn" style="text-decoration:none">{{ __('civinsis.auth.reset_volver') }}</a>
        </p>
      </div>
